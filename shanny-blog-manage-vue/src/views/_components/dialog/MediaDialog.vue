...
<template>
  <dialog class="modal h-full" :open="adminStore.mediaDialog">
    <div class="modal-box bg-base-200 w-3/4 max-w-3/4 h-14/15 overflow-visible">
      <form method="dialog">
        <button
          class="btn btn-sm btn-circle btn-ghost absolute right-2 top-2"
          @click="adminStore.closeDialog('media')"
        >
          ✕
        </button>
      </form>
      ...
      >
        <legend class="fieldset-legend">Add Media</legend>
        ...
        <div class="flex flex-col gap-2">
          <div class="flex flex-col md:flex-row md:gap-2">
            ...
            <div class="md:w-1/3">
              <label class="label w-full">Media</label>
              <input
                type="file"
